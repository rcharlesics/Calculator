...
GridView {
    ...
    Binding {
        ...
        value: {
            var value = 0;
            if (root.topToBottom)
                value = Math.ceil(root.count / Math.floor(root.height/root.itemHeight)) * root.itemWidth
            else
                value = Math.ceil(root.count / Math.floor(root.width/root.itemWidth)) * root.itemHeight
            return value;
        }
    }
}

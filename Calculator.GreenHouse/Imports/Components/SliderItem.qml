...
Item {
    ...
    property string backgroundSource: GreenHouseThemeManager.theme.asset("/placeholders/slider/background.png")
    //ASSET /placeholders/slider/knob.png
    property string knobSource: GreenHouseThemeManager.theme.asset("/placeholders/slider/knob.png")
    //ASSET
    property string fillSource: ""
    //ASSET
    property string bubbleSource: ""

    property bool touchPositioning: true
    property bool dynamicValueReporting: false
    ...
    Image {
        ...
        x: {
            if (!root.__vertical && !knobMa.drag.active)
                return ((knobMa.drag.maximumX - knobMa.drag.minimumX)
                        * ((internal.value - root.min) / Math.max(1, root.max - root.min))) - adjWidth
            return 0;
        }
        y: {
            if (root.__vertical && !knobMa.drag.active)
                return ((knobMa.drag.maximumY - knobMa.drag.minimumY)
                        * ((internal.value - root.min) / Math.max(1, root.max - root.min))) - adjHeight
            return 0;
        }
        z: 1
        ...
        onXChanged: {
            if (!root.__vertical && knobMa.drag.active) {
                internal.value = internal.valueForPosition(x + knobMa.handleCenter, root.width)
                if (root.dynamicValueReporting)
                    root.valueChangeRequested(internal.value)
            }
        }
        onYChanged: {
            if (root.__vertical && knobMa.drag.active && root.dynamicValueReporting) {
                internal.value = internal.valueForPosition(y + knobMa.handleCenter, root.height)
                if (root.dynamicValueReporting)
                    root.valueChangeRequested(internal.value)
            }
        }
    ...
    MouseArea {
        ...
        onMouseXChanged: {
            if (!knobMa.pressed && !root.__vertical && root.touchPositioning)
                root.valueChangeRequested(internal.valueForPosition(mouse.x, width))
        }
        onMouseYChanged: {
            if (!knobMa.pressed && root.__vertical && root.touchPositioning)
    ...
    onValueChangeRequested: internal.value = value
}
